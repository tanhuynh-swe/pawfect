...
- **Every video lives in `workspace/<slug>/`**, and its state is the set of files there: `brief.json` → `script.json` → `audio/`, `media/`, `shots/`, `narration.wav`, `captions.{ass,srt}`, `video_silent.mp4` → `final.mp4`, `thumbnail.jpg` → `review.json` (`approved`), `qa.json` (auto gate result), `published.json`. `status` and the queue are worked out by checking which files exist.
...
- **Stages:**
  ...
  - `render.py`: cuts to a new shot every `max_shot_seconds`, burns in ASS captions (retries with the `subtitles` filter when libass is missing), ducks the music and normalizes to −16 LUFS.
